...
// Validate inputs
if (empty($car_number) || empty($owner_name) || empty($parking_slot) || empty($package) || empty($payment_method) || empty($phone_number)) {
    echo json_encode(['success' => false, 'message' => 'All fields are required!']);
    exit();
}

// Check if the slot is already reserved or occupied
$query = "SELECT * FROM car_parking WHERE parking_slot = '$parking_slot' AND status IN ('reserved', 'occupied')";
...
if (mysqli_num_rows($result) > 0) {
    ...
    '>
        ...
        <form action='car-parking option ma.html' method='get'>
            <button style='
                padding: 12px 24px;
    ...
    $sql = "INSERT INTO car_parking (car_number, owner_name, parking_slot, package, status, payment_method, phone_number)
            VALUES ('$car_number', '$owner_name', '$parking_slot', '$package', 'reserved', '$payment_method', '$phone_number')";
