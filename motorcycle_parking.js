document.addEventListener("DOMContentLoaded", () => {
    ...
    function reserveSlot(slot) {
        if (slot.status === "available") {
            ...
            const motorcycleNumber = prompt("Enter your motorcycle number:");
            const ownerName = prompt("Enter your name:");
            if (carNumber && ownerName) {
                // Send data to backend to reserve slot
                fetch('motorcycle_parking.php', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    },
                    body: JSON.stringify({
                        motorcycle_number: "ABC123",
                        owner_name: "John Doe",
                        slot_number: 5
                    })
                })
    ...
    document.getElementById('check-form').addEventListener('submit', (e) => {
        ...
        fetch('check_location of motorcycle.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ motorcycle_number: motorcycleNumber }),
        })
        .then(response => response.json())
        .then(data => {
            ...
            document.getElementById('motorcycle-location').textContent = locationText;
        });
    });
