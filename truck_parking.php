...
// Validate inputs
//if (empty($truck_number) || empty($owner_name) || empty($parking_slot) || empty($package) || empty($payment_method) || empty($phone_number)) {
  //  echo json_encode(['success' => false, 'message' => 'All fields are required!']);
    //exit();
//}

// Check if the slot is already reserved or occupied
$query = "SELECT * FROM truck_parking WHERE parking_slot = '$parking_slot' AND status IN ('reserved', 'occupied')";
...
if (mysqli_num_rows($result) > 0) {
    ...
    '>
        ...
        <form action='truck parking option html page.html' method='get'>
            <button style='
                padding: 12px 24px;
    ...
    $sql = "INSERT INTO truck_parking (truck_number, owner_name, parking_slot, package, status, payment_method, phone_number)
            VALUES ('$truck_number', '$owner_name', '$parking_slot', '$package', 'reserved', '$payment_method', '$phone_number')";
